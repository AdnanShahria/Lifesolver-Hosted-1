...
export function useInventory() {
    ...
    const inventoryQuery = useQuery({
        ...
        queryFn: async () => {
            if (!userId) return [];
            return await apiFetch("/api/dynamic/inventory");
        },
        enabled: !!userId,
        staleTime: 1000 * 60 * 2,
    });
    ...
    const addItem = useMutation({
        mutationFn: async (item: Omit<InventoryItem, "id" | "user_id" | "finance_entry_id"> & { record_purchase?: boolean, finance_category?: string }) => {
            ...
            if (item.record_purchase && item.cost && item.cost > 0) {
                const finRes = await apiFetch("/api/finance", {
                    method: "POST",
                    body: JSON.stringify({
                        type: 'expense',
                        amount: item.cost,
            ...
            const res = await apiFetch("/api/dynamic/inventory", {
                method: "POST",
                body: JSON.stringify({
    ...
    const updateItem = useMutation({
        mutationFn: async (item: InventoryItem) => {
            await apiFetch("/api/dynamic/inventory", {
                method: "PUT",
                body: JSON.stringify({
                    id: item.id,
                    item_name: item.item_name,
    ...
    const deleteItem = useMutation({
        mutationFn: async (id: string) => {
            await apiFetch(`/api/dynamic/inventory/${id}`, { method: "DELETE" });
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["inventory"] }),
    });
