...
export default function VerifyOtpPage() {
    ...
    const { verifyOtp, register, isAuthenticated } = useAuth();
                        ...
                        <div className="flex justify-between gap-2 sm:gap-3">
                            {otp.map((digit, index) => (
                                <motion.input
                                    ...
                                    onPaste={handlePaste}
                                    className={`w-11 h-14 sm:w-12 sm:h-15 text-center text-2xl font-bold backdrop-blur-md border-2 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/20 transition-all duration-300 ${
                                        digit 
                                        ? "border-yellow-500 text-yellow-600 dark:text-yellow-400 shadow-[0_0_12px_rgba(234,179,8,0.25)] bg-yellow-50/80 dark:bg-yellow-950/30" 
                                        : "border-yellow-300/80 dark:border-yellow-900/40 bg-yellow-50/40 dark:bg-yellow-950/15 text-foreground"
                                    }`}
                                />
                            ))}
                        </div>
