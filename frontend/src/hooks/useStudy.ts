...
export function useStudy() {
    ...
    const subjectsQuery = useQuery<StudySubject[]>({
        ...
        queryFn: async () => {
            if (!userId) return [];
            return await apiFetch("/api/dynamic/study_subjects");
        },
        enabled: !!userId && qc.isFetching({ queryKey: ["appData", userId] }) === 0,
        initialData: () => {
            const appData = qc.getQueryData<any>(["appData", userId]);
            return appData?.study_subjects;
        },
        staleTime: 1000 * 60 * 2,
    });
    ...
    const chaptersQuery = useQuery<StudyChapter[]>({
        ...
        queryFn: async () => {
            if (!userId) return [];
            return await apiFetch("/api/dynamic/study_chapters_v2");
        },
        enabled: !!userId && qc.isFetching({ queryKey: ["appData", userId] }) === 0,
        initialData: () => {
            const appData = qc.getQueryData<any>(["appData", userId]);
            return appData?.study_chapters_v2;
        },
        staleTime: 1000 * 60 * 2,
    });
    ...
    const partsQuery = useQuery<StudyPart[]>({
        ...
        queryFn: async () => {
            if (!userId) return [];
            return await apiFetch("/api/dynamic/study_parts");
        },
        enabled: !!userId && qc.isFetching({ queryKey: ["appData", userId] }) === 0,
        initialData: () => {
            const appData = qc.getQueryData<any>(["appData", userId]);
            return appData?.study_parts;
        },
        staleTime: 1000 * 60 * 2,
    });
    ...
    const commonPresetsQuery = useQuery<StudyCommonPreset[]>({
        ...
        queryFn: async () => {
            if (!userId) return [];
            return await apiFetch("/api/dynamic/study_common_presets");
        },
        enabled: !!userId && qc.isFetching({ queryKey: ["appData", userId] }) === 0,
        initialData: () => {
            const appData = qc.getQueryData<any>(["appData", userId]);
            return appData?.study_common_presets;
        },
        staleTime: 1000 * 60 * 2,
    });
    ...
    const addSubject = useMutation({
        mutationFn: async (name: string) => {
            ...
            const res = await apiFetch("/api/dynamic/study_subjects", {
                method: "POST",
    ...
    const renameSubject = useMutation({
        mutationFn: async ({ id, name }: { id: string; name: string }) => {
            await apiFetch("/api/dynamic/study_subjects", {
                method: "PUT",
        ...
        mutationFn: async (id: string) => {
            ...
            for (const ch of subjectChapters) {
                ...
                for (const p of chapterParts) {
                    await apiFetch(`/api/dynamic/study_parts/${p.id}`, { method: "DELETE" });
                }
                await apiFetch(`/api/dynamic/study_chapters_v2/${ch.id}`, { method: "DELETE" });
            }
            await apiFetch(`/api/dynamic/study_subjects/${id}`, { method: "DELETE" });
        },
        onSuccess: invalidateAll,
    });
    ...
    const addChapter = useMutation({
        mutationFn: async ({ subjectId, name }: { subjectId: string; name: string }) => {
            ...
            const res = await apiFetch("/api/dynamic/study_chapters_v2", {
                method: "POST",
            ...
            const createPartsForPresets = async (presets: StudyCommonPreset[], parentPartId: string | null = null) => {
                for (const preset of presets) {
                    const presetRes = await apiFetch("/api/dynamic/study_parts", {
                        method: "POST",
                        body: JSON.stringify({
                            chapter_id: id,
                            name: preset.name,
    ...
    const renameChapter = useMutation({
        mutationFn: async ({ id, name }: { id: string; name: string }) => {
            await apiFetch("/api/dynamic/study_chapters_v2", {
                method: "PUT",
                body: JSON.stringify({ id, name })
            });
        },
    ...
    const deleteChapter = useMutation({
        mutationFn: async (id: string) => {
            ...
            await apiFetch(`/api/dynamic/study_chapters_v2/${id}`, { method: "DELETE" });
        },
        onSuccess: invalidateAll,
    });
    ...
    const addPart = useMutation({
        mutationFn: async ({ chapterId, name, estimatedMinutes, scheduledDate, scheduledTime, parentId }: {
            ...
            const res = await apiFetch("/api/dynamic/study_parts", {
                method: "POST",
                body: JSON.stringify({
    ...
    const updatePart = useMutation({
        mutationFn: async (update: { id: string; name?: string; estimated_minutes?: number; scheduled_date?: string | null; scheduled_time?: string | null; notes?: string | null }) => {
            ...
            if (Object.keys(body).length > 1) {
                await apiFetch("/api/dynamic/study_parts", {
                    method: "PUT",
    ...
    const deletePart = useMutation({
        mutationFn: async (id: string) => {
            await apiFetch(`/api/dynamic/study_parts/${id}`, { method: "DELETE" });
        },
        onSuccess: invalidateAll,
    });
    ...
    const addCommonPreset = useMutation({
        mutationFn: async ({ subjectId, name, minutes, parentId, type = "chapter" }: { subjectId: string; name: string; minutes: number, parentId?: string, type?: "chapter" | "part" }) => {
            const res = await apiFetch("/api/dynamic/study_common_presets", {
                method: "POST",
                body: JSON.stringify({
    ...
    const deleteCommonPreset = useMutation({
        mutationFn: async (id: string) => {
            await apiFetch(`/api/dynamic/study_common_presets/${id}`, { method: "DELETE" });
        },
        onSuccess: () => qc.invalidateQueries({ queryKey: ["study_common_presets"] }),
    });
