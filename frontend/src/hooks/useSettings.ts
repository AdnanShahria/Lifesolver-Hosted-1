...
export interface UserSettings {
    id: string;
    user_id: string;
    theme: "light" | "dark" | "oled" | "midnight" | "nord" | "mocha";
    currency: string;
    language: string;
...
export function useSettings() {
    ...
    const settingsQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            const result = await apiFetch("/api/dynamic/settings");
            
            if (!result || result.length === 0) {
                // Create default settings if not exist
                const res = await apiFetch("/api/dynamic/settings", {
                    method: "POST",
                    body: JSON.stringify({
                        theme: "dark",
                        currency: "BDT",
            ...
            return {
                ...
                theme: (row.theme as "light" | "dark" | "oled" | "midnight" | "nord" | "mocha") || "dark",
    ...
    const updateSettings = useMutation({
        mutationFn: async (updates: Partial<UserSettings> & { id?: string }) => {
            ...
            if (Object.keys(body).length > 0 && body.id) {
                await apiFetch("/api/dynamic/settings", {
                    method: "PUT",
