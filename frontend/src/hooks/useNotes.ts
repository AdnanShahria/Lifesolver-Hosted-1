...
export function useNotes() {
    ...
    const notesQuery = useQuery({
        ...
        queryFn: async () => {
            if (!userId) return [];
            const result = await apiFetch("/api/dynamic/notes");
        ...
        initialData: () => {
            ...
            if (!appData?.notes) return undefined;
            return appData.notes.map((n: any) => ({
                ...n,
                is_pinned: n.is_pinned ?? 0,
                color: (n.color as NoteColor) || "default",
                is_archived: n.is_archived ?? 0,
    ...
    const addNote = useMutation({
        mutationFn: async (note: { title: string; content?: string; tags?: string; color?: NoteColor }) => {
            ...
            const res = await apiFetch("/api/dynamic/notes", {
                method: "POST",
                body: JSON.stringify({
                    title: note.title,
    ...
    const updateNote = useMutation({
        mutationFn: async (note: Partial<Note> & { id: string }) => {
            ...
            await apiFetch("/api/dynamic/notes", {
                method: "PUT",
    ...
    const deleteNote = useMutation({
        mutationFn: async (id: string) => {
            await apiFetch(`/api/dynamic/notes/${id}`, { method: "DELETE" });
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ["notes"] }),
    });
